...
name = "./logs/log%s.txt" % (time.strftime("%Y-%m-%d", time.localtime()))
...
class Spider(object):
    ...
    def save_data(self, data):
        print(data)
        logging.info(data)
        res = requests.post(url="http://pgkoc.dev.max-digital.cn/koc/scoreverify/save_Url", data=data)
        print(111, res)
        print(res.json())
    ...
    def get_url(self, page=1):
        ...
        res = requests.post(url='http://pgkoc.dev.max-digital.cn/koc/scoreverify/url_Day',
                            data=data)
        data = res.json()
        print(data)
        logging.info(data)
